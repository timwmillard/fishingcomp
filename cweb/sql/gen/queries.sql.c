...
int sql_CreateCompetitor(sql_context *ctx, CreateCompetitorParams arg, Competitor *result) {

    const char *sql = "-- name: CreateCompetitor :one\n"
                      "insert into competitor (\n"
                      "    first_name,\n"
                      "    last_name,\n"
                      "    email\n"
                      ") values ( ?, ?, ?)\n"
                      "returning id, competitor_no, first_name, last_name, email, mobile, phone, address1, address2, suburb, state, postcode, boat_id\n";
...
int sql_GetCompetitor(sql_context *ctx, sql_int64 id, Competitor *result) {
    ...
    const char *sql = "-- name: GetCompetitor :one\n"
                      "select id, competitor_no, first_name, last_name, email, mobile, phone, address1, address2, suburb, state, postcode, boat_id\n"
                      "from competitor\n"
                      "where id = ?\n";
...
int sql_ListCompetitors(sql_context *ctx, Competitor **result, int *result_count) {

    const char *sql = "-- name: ListCompetitors :many\n"
                      "select id, competitor_no, first_name, last_name, email, mobile, phone, address1, address2, suburb, state, postcode, boat_id\n"
                      "from competitor\n";
